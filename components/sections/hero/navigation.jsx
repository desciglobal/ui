  ...
  return (
    <nav className='sticky top-0 bg-white h-9 z-30'>
      <div class="lg:grid grid-col-6 grid-flow-col gap-4 h-5 items-center text-sm hidden ">
        <div className='h-full w-full pl-4'><Logo /></div>
  ...
  return (
    <div className="fixed z-50 top-0 left-0 w-full h-full bg-[#1231D5] text-lg overflow-y-auto">
      <div className="flex justify-between items-center mb-6">
        <Logo />
        <button className="mr-2 text-white" onClick={onToggle}>Close -</button>
      </div>
      <div className="text-white/[.60]">
        <MenuItem subitems={<>
          <div onClick={goTo('highlights')} className="py-3 px-2">Highlighted this week</div>
          <div onClick={goTo('upcoming')} className="py-3 px-2 flex items-center">Upcoming<div className="rounded-full bg-red-600 w-2.5 h-2.5 mx-2"></div></div>
          <div onClick={goTo('contribute')} className="py-3 px-2">Submit new event</div>
        </>} onClick={goTo('events')}>Events</MenuItem>
